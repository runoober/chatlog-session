...
const handleSendToAI = () => {
  ...
  try {
    ...
    // 获取第一条消息的会话 ID（假设所有消息都来自同一会话）
    const firstMessage = batchStore.getSelectedMessages[0]
    if (!firstMessage) {
    ...
    // 添加到引用列表
    referenceStore.addMessageReferences(messageIds, firstMessage.talker)
    
    ElMessage.success(`已添加 ${selectedCount.value} 条消息到 AI 引用`)
...
const handleDelete = () => {
  ...
  ElMessageBox.confirm(
    `确定要删除选中的 ${selectedCount.value} 条消息吗？`,
    '删除确认',
    {
      ...
      <el-dropdown trigger="click" @command="handleCommand">
        ...
        <template #dropdown>
          <el-dropdown-menu>
            ...
            <!-- 发送到 AI -->
            <el-dropdown-item command="sendToAI" :icon="ChatDotRound">
              发送到 AI
            </el-dropdown-item>
